...
## Tool 12: generate_threat_detection_opportunity (generate-tdo)
Generates a Threat Detection Opportunity (TDO) for a given threat, which can be a GTI campaign, or a new threat described by the user from any external source. It returns a structured description containing MITRE information, observed contextual IOCs, procedures, and log types.

**Usage:**
```bash
<PATH_TO_SECOPS_SKILLS>/venv/bin/python <PATH_TO_SECOPS_SKILLS>/src/secops.py detection generate-tdo --threat "<THREAT_DESCRIPTION_OR_IDENTIFIER>"
```

**Options:**
- `--threat`: Required. Description or details of the threat (e.g. a GTI campaign name or custom description).
...
This tool is essential for determining if a threat scenario, represented by synthetic UDM events (usually generated by `generate_synthetic_events`), is already covered by existing detection content. 

**Usage:**
```bash
<PATH_TO_SECOPS_SKILLS>/venv/bin/python <PATH_TO_SECOPS_SKILLS>/src/secops.py detection evaluate-rule-coverage --udms-json "<JSON_ARRAY>"
```

**Options:**
- `--udms-json`: Required. A JSON array of stringified UDM events to evaluate.
